<script lang="ts">
	...
	function onScanComplete(e) {
		console.log(e);
		if (!e.detail) return;
		let scannedString = e.detail;
		if (scannedString.startsWith('TOKEN|')) {
			let loginToken = scannedString.split('TOKEN|')[1];
			console.log('SCAN: LOGIN TOKEN:', loginToken);
			token = loginToken;
			handleLoginClick();
		}
	}
	...
	onMount(() => {
		...
		setInterval(() => {
			timeSinceActivity =
				Date.now() - JSON.parse(window.sessionStorage.getItem('lastActivity') || '0');
		}, 500);
	});
</script>
...
<div class="relative px-8">
	...
	>
		...
		<NavUl {hidden}>
			<!-- {#each Object.entries(routes) as [route, routeDetail]}
				{#if routeDetail?.props?.name}
					<NavLi
						on:click={() => navigateTo(route.split('/:', 2)[0])}
						active={$location.includes(route.split('/:', 2)[0])}
						>{routeDetail?.props?.name}<span class="text-xs align-super text-orange-600"
							>{(prioritiesVisible && routeDetail?.props?.priority) || ''}</span
						></NavLi
					>
				{/if}
			{/each} -->
			...
			<div>
				<div class="flex items-center justify-center">
					<img
						...
						src={'http://bwipjs-api.metafloor.com/?bcid=datamatrix&text=' + currentSerial}
					/>
				</div>
				<p alt={parseInt(currentSerial, 10)}>{currentSerial}</p>
			</div>
		</Button>
		...
		<Dropdown inline bind={userVisible}>
			...
			{#if false}
				...
				>
				<DropdownItem slot="footer" on:click={() => {}}>Sign out</DropdownItem>
			{/if}
		</Dropdown>
	</Navbar>
</div>
